...
namespace IngameScript
{
	partial class Program : MyGridProgram
	{
		...
		//specific initializer for setup
		public void GYROInit()
		{
			//Apply values from ShipAlignment Programm
			GYROcontrollerName = RemoteControllName;
			GYROdirection = Direction;
			GYROgravityMode = new Toggle(stateOfProperty: GravityMode);
			GYROgyroLimit = GyroscopeLimit;
			GYROminAngleRad = MinimumAngle;
			GYROctrl_coeff = ControlCoefficient;
			...
			if (controllersList.Count() < 1) throw new SEE.MissingBlockException($"(Error) No 'Remote Controll' with name '{GYROcontrollerName}' found", priority : true);
		...
		//run function to apply logic
		public void GYRORun()
		{
			if (GYROready)
			{
				if (GYROgravityMode.state)
				{
					//Gravity Mode
